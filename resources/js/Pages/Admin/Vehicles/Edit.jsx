            ...
            header={
                <div className="flex items-center gap-4">
                    <Link
                        ...
                        className="p-2 hover:bg-navy/5 rounded-xl transition-all text-navy/40 hover:text-navy"
                    >
                        <ArrowLeft size={20} />
                    </Link>
                    <h2 className="text-xl font-bold leading-tight text-navy">
                        Edit Armada: {vehicle.name}
                    </h2>
                </div>
            }
        >
            ...
            <div className="py-12">
                <div className="mx-auto max-w-4xl sm:px-6 lg:px-8">
                    <form
                        onSubmit={submit}
                        className="bg-white shadow-xl sm:rounded-3xl border border-navy/5 overflow-hidden"
                    >
                        <div className="p-8 space-y-8">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div className="space-y-2">
                                    ...
                                    {errors.seat_capacity && (
                                        <p className="text-xs text-red-500">{errors.seat_capacity}</p>
                                    )}
                                </div>
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {data.facilities.map((facility, index) => (
                                        <span
                                            key={index}
                                            className="flex items-center gap-2 px-4 py-2 bg-gold/10 text-gold rounded-xl text-sm font-bold border border-gold/20"
                                        >
                                            {facility}
                                            <button
                                                type="button"
                                ...
                                {errors.facilities && (
                                    <p className="text-xs text-red-500">{errors.facilities}</p>
                                )}
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div className="space-y-4">
                                    <label className="text-sm font-bold text-navy uppercase tracking-wider">
                                        Foto Utama (Kosongkan jika tetap)
                                    </label>
                                    <div className="relative border-2 border-dashed border-navy/10 rounded-3xl p-8 text-center hover:border-gold/50 transition-colors group cursor-pointer">
                                        ...
                                        />
                                        <Upload
                                            className="mx-auto mb-4 text-navy/20 group-hover:text-gold transition-colors"
                                            size={40}
                                        />
                                        <p className="text-sm text-navy/60">
                                            {data.photo
                                                ? data.photo.name
                                                : 'Klik untuk ganti foto (Max 2MB)'}
                                        </p>
                                    </div>
                                    {vehicle.photo && (
                                        <p className="text-[10px] text-navy/40 italic">
                                            File saat ini: {vehicle.photo.split('/').pop()}
                                        </p>
                                    )}
                                    {errors.photo && (
                                ...
                                <div className="space-y-4">
                                    <label className="text-sm font-bold text-navy uppercase tracking-wider">
                                        File PDF Spesifikasi (Kosongkan jika tetap)
                                    </label>
                                    <div className="relative border-2 border-dashed border-navy/10 rounded-3xl p-8 text-center hover:border-gold/50 transition-colors group cursor-pointer">
                                        <input
                                            type="file"
                                        ...
                                        <p className="text-sm text-navy/60">
                                            {data.pdf_file
                                                ? data.pdf_file.name
                                                : 'Klik untuk ganti PDF (Max 5MB)'}
                                        </p>
                                    </div>
                                    {vehicle.pdf_file && (
                                        <p className="text-[10px] text-navy/40 italic">
                                            File saat ini: {vehicle.pdf_file.split('/').pop()}
                                        </p>
                                    )}
                                    {errors.pdf_file && (
                                        <p className="text-xs text-red-500">{errors.pdf_file}</p>
                                    )}
                                </div>
                            </div>
                        ...
                        <div className="bg-navy/5 p-8 flex justify-between items-center group">
                            <p className="text-sm text-navy/40 italic">
                                Tinjau kembali data sebelum memperbarui armada.
                            </p>
                            <button
                                type="submit"
                                disabled={processing}
