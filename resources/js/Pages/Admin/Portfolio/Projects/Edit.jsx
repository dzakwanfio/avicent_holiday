...
export default function Edit({ project }) {
    const { data, setData, post, processing, errors } = useForm({
        _method: 'PUT',
        title: project.title || '',
        category: project.category || 'Wisata Religi',
        image: null,
    });
                    ...
                    <form onSubmit={submit} className="bg-white shadow-xl sm:rounded-3xl border border-navy/5 overflow-hidden">
                        <div className="p-8 space-y-6">
                            ...
                            <div className="space-y-4">
                                <label className="text-sm font-bold text-navy uppercase tracking-wider">Foto Proyek (Kosongkan jika tetap)</label>
                                <div className="mb-4 aspect-video rounded-2xl overflow-hidden bg-navy/5">
                                ...
                                <div className="relative border-2 border-dashed border-navy/10 rounded-3xl p-12 text-center hover:border-gold/50 transition-colors group cursor-pointer">
                                    ...
                                    <p className="text-sm text-navy/60">
                                        {data.image ? data.image.name : 'Klik untuk ganti foto proyek (Max 2MB)'}
                                    </p>
                                </div>
